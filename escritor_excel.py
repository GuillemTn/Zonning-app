...
def _extraer_colores_empleados(ruta_horario='Horario semana.xlsx'):
    """
    Extrae el color exacto de fondo de cada empleado desde el archivo de horarios.
    Retorna: dict {nombre_empleado: color_rgb}
    """
    colores = {}
    if isinstance(ruta_horario, str) and not os.path.exists(ruta_horario):
        return colores
    try:
        if hasattr(ruta_horario, 'seek'):
            ruta_horario.seek(0)
...
def _leer_tabla_mapeo_estilos(ruta_plantilla='Plantilla_Visual.xlsx', hoja='Config_Formatos'):
    ...
    """
    if isinstance(ruta_plantilla, str) and not os.path.exists(ruta_plantilla):
        raise FileNotFoundError(f"Plantilla no encontrada: {ruta_plantilla}")

    if hasattr(ruta_plantilla, 'seek'):
        ruta_plantilla.seek(0)
...
def generar_salida_semanal(empleados_por_dia, cuadrantes_por_dia, ruta_salida='Zonning_Semanal_Completo.xlsx', ruta_plantilla='Plantilla_Visual.xlsx', ruta_horario='Horario semana.xlsx'):
    """
    ...
    # Convertir a ruta absoluta si es relativa y es string (no aplicable a buffers)
    if isinstance(ruta_salida, str) and not os.path.isabs(ruta_salida):
    ...
    try:
        ...
        # Extraer mapa de colores originales por empleado desde el archivo de horarios
        colores_map = _extraer_colores_empleados(ruta_horario)
        ...
        for dia, cuadrante_dict in cuadrantes_por_dia.items():
            ...
            for r_idx, (_, row) in enumerate(df.iterrows(), start=2):
                ...
                # Aplicar color extraído exactamente de Horario semana.xlsx
                emp_name_str = str(row['Empleado']).strip()
                color_rgb = colores_map.get(emp_name_str)
                # Se omite '00000000' (valor transparente genérico)
                if color_rgb and color_rgb != '00000000':
                    try:
                        # Si el color es válido, instanciamos el color dinámico clonando su RGB
                        emp_cell.fill = PatternFill(start_color=color_rgb, end_color=color_rgb, fill_type="solid")
                    except Exception:
        ...
        # Intentar guardar, si hay conflicto, usar nombre alternativo (ignorado si es buffer)
        intento = 0
        ruta_final = ruta_salida
        while intento < 5:
            try:
                if isinstance(ruta_final, str) and os.path.exists(ruta_final) and intento == 0:
                    try:
                ...
                wb.save(ruta_final)
                if isinstance(ruta_final, str):
                    print(f"\n[OK] Archivo semanal generado: {ruta_final}")
                else:
                    print(f"\n[OK] Archivo semanal generado en buffer")
                return True
                ...
                if intento < 5:
                    ...
                    nombre_base = os.path.basename(ruta_salida).replace('.xlsx', '')
                    ruta_final = os.path.join(directorio, f"{nombre_base}_temp_{intento}.xlsx")
                    continue
                else:
                    raise
        return False
